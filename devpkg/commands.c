...
int Command_depends(apr_pool_t *p,const char *path){
	...
	for(line = bgets((bNgetc)fgetc ,in, '\n'); line !=NULL ;line = bgets((bNgetc)fgetc ,in,'\n')){
		btrimws(line); // delete the whitespace at the beginning and the end of the string
...
int Command_fetch(apr_pool_t *p,const char *url,int fetch_only){
	...
	if(apr_fnmatch(GIT_PAT,info.path,0)==APR_SUCCESS){
		...
		log_info("Building accord to DEPENDS: %s",url);
		rv = Command_depends(p,depends_file);
		check(rv ==0,"Failed to process the DEPENDS: %s",url);
		//indicate that no installation is needed
		return 0;
		...
		check(rv ==APR_SUCCESS,"Failed to untar %s",TAR_GZ_SRC);
		...
		sentinel("Dont know how to handle %s",url);
	}
	//indicate that an install needs to actually run
	return 1;
	error:
	return -1;
}

...
static bstring DB_load(){
	...
	data = bread((bNread)fread,db); // bNread?
	check(data,"failed to read from db_file %s",DB_FILE);
	DB_close(db);
	return data;
	error:
	if(db)
...
int DB_init(){
	...
	if(access(DB_DIR,W_OK | X_OK)==-1){
		apr_status_t rc =apr_dir_make_recursive(DB_DIR,APR_FPROT_UREAD|APR_FPROT_UWRITE|APR_FPROT_UEXECUTE|APR_FPROT_GREAD|APR_FPROT_GWRITE|APR_FPROT_GEXECUTE,p);

...
#include "db.h"
#include "commands.h"
int main(int argc,const char const*argv[]){ //bro this is confusing af, so 1st const indicates that the data is constant, and the 2nd is that pointer is const. 
	apr_pool_t *p =NULL;
	apr_pool_initialize();
	...
	switch(request){
		case COMMAND_INSTALL:
			check(url,"you must give at least an URL");
		...
		case COMMAND_BUILD:
			...
			break;
		case COMMAND_FETCH:
			check(url !=NULL,"Give an URL to check");
		...
		case COMMAND_INIT:
			...
			break;
		default:
			sentinel("Invalid commands given");
	}
	return 0;
	error:
	return -1;

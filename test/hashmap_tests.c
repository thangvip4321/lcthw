...
struct tagbstring expect1 = bsStatic("THE value1");
struct tagbstring expect2 = bsStatic("the value2");
struct tagbstring expect3 = bsStatic("the value3");
...
char* test_delete(){
	...
	mu_assert(deleted !=NULL,"got null on delete");
	mu_assert(deleted == &expect3,"should get  test1");
	result =Hashmap_get(map,&test3);
	mu_assert(result ==NULL,"should delete");
	return NULL;
}

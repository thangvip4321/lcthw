...
#include <time.h>
struct tagbstring IN_STR = bsStatic("I have ALPHA beHP ALPHA and orange ALPHA");
struct tagbstring ALPHA;
...
char *test_find_and_scan(){
	...
	//mu_assert((scan_i ==find_i),"find and scan dont match");
...
char* test_find_performance(){
	...
	for(i =0;i<500;i++){
		...
		mu_assert((found_at != -1),"failed to find");
	}
	time_t start = time(NULL);
	do{
		for(i=0;i<1000;i++){
...
char* all_tests(){
	struct bstrList *List = bsplit(&IN_STR,' ');
	ALPHA = *(List->entry[2]);
	debug("%s",ALPHA.data);
	mu_suite_start();

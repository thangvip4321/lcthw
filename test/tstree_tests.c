...
char *test_insert(){
	...
	node = TSTree_insert(node,bdata(&test2),blength(&test2),valueB);
	mu_assert((node !=NULL),"failed to insert into tst iwth second name");
	node = TSTree_insert(node,bdata(&test3),blength(&test3),reverse);
	mu_assert((node !=NULL),"failed to insert into tst iwth reverse name");
	node = TSTree_insert(node,bdata(&test4),blength(&test4),value4);
	mu_assert((node !=NULL),"failed to insert into tst iwth fourth name");
	return NULL;
}
...
char* test_search_prefix(){
	...
	mu_assert((res !=NULL),"should find short prefix");
	res = TSTree_search_prefix(node,"TE--",strlen("TE--"));
	mu_assert((res !=NULL),"should find partial prefix");
	return NULL;
}
...
char* test_traverse(){
	...
	mu_assert((traverse_count ==4),"Didnt find 4 keys");
	return NULL;
}
char* test_delete(){
	int a =TSTree_delete(node,bdata(&test1),blength(&test1));
	mu_assert((a==0),"wtf is wrong with delete function");
	a = TSTree_delete(node,"TT",2);
	mu_assert((a!=0),"wtf is wrong with delete function again?");
	return NULL;
}
char* test_destroy(){
	TSTree_destroy(node);
